...
public class JSON {
    ...
    /**
     * <p>Constructor with a string value.</p>
     * <p><b>Note: </b> If you want to decode a json string,
     * use JSON.decode(String) instead of new JSON(String).</p>
     * @param value
     */
    ...
    /**
     * <p>Constructor with another JSON object.</p>
     * <p><b>Note: </b>It's not deep copy:
     * please use JSON.deepCopy(JSON) if you want.</p>
     * @param another
     */
    ...
    /**
     * <p>Constructor with a list value.</p>
     * @param list
     */
    ...
    /**
     * Get int value if it's an integer or it can parse to an integer,
     * else return null
     * @return an integer, or null
     */
    public Integer getInt() {
        if (type==TYPE.INTEGER) return intValue;
        if (type==TYPE.DOUBLE) return (int)doubleValue;
    ...
    /**
     * Get double value if it's a double or it can parse to a double,
     * else return null
     * @return a double, or null
     */
    public Double getDouble() {
        if (type==TYPE.DOUBLE) return doubleValue;
        if (type==TYPE.INTEGER) return intValue+0.0;
        if (type==TYPE.BOOLEAN) return boolValue?1.0:0.0;
        if (type==TYPE.STRING) {
            try {
    ...
    /**
     * Get boolean value if it's a boolean or it can parse to a boolean,
     * else return null
     * @return a boolean, or null
     */
    public Boolean getBoolean() {
        if (type==TYPE.BOOLEAN) return boolValue;
        if (type==TYPE.DOUBLE) return doubleValue!=0;
        if (type==TYPE.INTEGER) return intValue!=0;
        if (type==TYPE.STRING) {
            try {
    ...
    /**
     * Get string value if it's a string or it can parse to a string,
     * else return null
     * @return a string, or null
     */
    public String getString() {
        if (type==TYPE.STRING) return decodeString(stringValue);
        if (type==TYPE.INTEGER) return String.valueOf(intValue);
        if (type==TYPE.DOUBLE) return String.valueOf(doubleValue);
        if (type==TYPE.BOOLEAN) return String.valueOf(boolValue);
        return null;
    }
    ...
    public static JSON decode(String string, boolean useSingleQuote) {
        ...
        for (int i=0;i<string.length();i++) {
            ...
            if (key==null||state!=2) return null;
            if (c=='[' || c=='{') {
                int np=i;
            ...
            tmp=tmp.trim();
            if ("true".equals(tmp)) map.put(key, new JSON(true));
            else if ("false".equals(tmp)) map.put(key, new JSON(false));
            else {
                if (tmp.isEmpty()) return null;
                if (!tmp.matches("[+-]?\\d*(.\\d*)?")) return null;
                try {
                    if (tmp.contains(".")) map.put(key, new JSON(Double.parseDouble(tmp)));
                    else map.put(key, new JSON(Integer.parseInt(tmp)));
                }
        ...
        if (inQuote || state==1 || state==2) return null;
        return new JSON(map);
    }
    ...
    public static JSON deepCopy(JSON another) {
        ...
        if (another.isBasicType()) return new JSON(another);
        JSON json=new JSON();
    ...
    /**
     * Encode JSON object to string. If needUrlEncoded, special characters such as
     * `"`, `\n`, or non-ascii will be url-encoded.
     * @param json
     * @param needUrlEncode whether to encode
    ...
    /**
     * <p>Show a human-readable structure of the json object.
     * Strings will be decoded, so there may be extra `"`, `\n` or other
     * special characters.</p>
     * <p>Use <code>{@link #toString()}</code> to
     * encode JSON to a standard JSON string.</p>
     * @return
     */
    ...
    private String toStringHumanReadable(String prefix, boolean newLine) {
        if (isBasicType()) {
            if (type==TYPE.STRING) {
                return (newLine?prefix:"")+"\""+decodeString(stringValue)+"\"";
            }
            return (newLine?prefix:"")+getString();
        }
        if (type==TYPE.LIST) {
            StringBuilder builder=new StringBuilder();
            if (listValue.size()==0) return (newLine?prefix:"")+"[]";
            if (newLine) builder.append(prefix);
            builder.append("[");
            boolean added=false;
    ...
    public String toString(boolean needUrlEncode) {
        if (isBasicType()) {
            if (type==TYPE.STRING) {
                if (needUrlEncode)
                    return "\""+stringValue+"\"";
                else return "\""+decodeString(stringValue)+"\"";
            }
            return getString();
        }
        if (type==TYPE.LIST) {
        ...
        for (Map.Entry<String, JSON> entry: map.entrySet()) {
            ...
            if (entry.getValue()==null) builder.append("null");
            else builder.append(entry.getValue().toString(needUrlEncode));
            builder.append(",");
        }
        builder.deleteCharAt(builder.length()-1);
        builder.append("}");
